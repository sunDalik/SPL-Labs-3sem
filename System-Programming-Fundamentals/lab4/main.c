...
const char *UNKNOWN_OPTION_MESSAGE = "Invalid option\n";
    ...
    while ((rez = getopt(argc, argv, "ai")) != -1) {
        switch (rez) {
            ...
            case 'i':
                ...
                break;
            case '?':
                write(2, UNKNOWN_OPTION_MESSAGE, strlen(UNKNOWN_OPTION_MESSAGE));
                write(2, USAGE_MESSAGE, strlen(USAGE_MESSAGE));
                return 1;
        }
    }

    const int FILE_CNT = argc - optind;
    FILE **files = malloc(FILE_CNT * sizeof(int));
    int f_i = 0;
